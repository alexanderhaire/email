...
def get_low_raw_materials() -> list:
    ...
    for _, row in df.iterrows():
        ...
        if gp_order_point > 0 and qty_available_alert < gp_order_point:
            # Override urgency if it's "OK" because our strict logic (ignoring On Order) says it's critical
            if row['urgency'] == 'OK':
...
def main():
    ...
    # Startup Logic: Check for existing state to prevent spam on fresh install/reset
    if not STATE_FILE.exists():
        LOGGER.info(f"State file not found at {STATE_FILE}")
        LOGGER.info("Initializing baseline state from current low items...")
        
        try:
            current_low_items = get_low_raw_materials()
            initial_state = {item.item_number: datetime.now().isoformat() for item in current_low_items}
            save_state(initial_state)
            LOGGER.info(f"Baseline initialized with {len(initial_state)} items. Alerts suppressed for these existing items.")
            LOGGER.info("Alerts will only be sent for NEW items dropping below reorder point from now on.")
        except Exception as e:

...
class ReorderRecommendation:
    ...
    days_of_coverage: float     # available / seasonal_burn_rate
...
def calculate_days_of_coverage(qty_available: float, burn_rate: float) -> float:
    ...
    Args:
        qty_available: Available quantity (on_hand - allocated)
        burn_rate: Effective daily burn rate (use seasonal_burn_rate for accuracy)
    
    Returns:
        Days until stockout (999 if no burn)
    """
    if burn_rate <= 0:
...
def _compute_seasonal_burn_rate(
    ...
    Uses the same algorithm as the dashboard (market_insights.calculate_seasonal_burn_metrics):
...
def get_reorder_recommendations(
    ...
    for row in rows:
        ...
        # Use seasonal burn rate for coverage so the alert matches the dashboard
        seasonal_burn_rate, seasonal_factor = _compute_seasonal_burn_rate(
            cursor, item_number, avg_daily_usage, location
        )
        effective_burn = seasonal_burn_rate if seasonal_burn_rate > 0 else avg_daily_usage
        days_of_coverage = calculate_days_of_coverage(qty_available, effective_burn)
